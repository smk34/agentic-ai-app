...
//                               className="flex items-center space-x-2 px-3 py-1 bg-orange-900/20 border border-orange-800 rounded-lg hover:bg-orange-900/30 transition-colors text-sm text-orange-300"
//                             >
//                               <span>
    ...
    <div className="min-h-screen bg-black relative overflow-hidden">
      ...
      <div className="relative z-10 px-6 py-10 max-w-7xl mx-auto">
        ...
        <div className="mb-10">
          ...
          <p className="text-zinc-400 text-lg">Manage and analyze your contact center transcriptions</p>
        </div>
        ...
        <div className="mb-12">
          <div className="bg-zinc-900/50 backdrop-blur-sm rounded-2xl p-8 border border-zinc-800 shadow-2xl">
            <div className="mb-6">
              ...
              <p className="text-zinc-400">Upload and transcribe your contact center audio files</p>
            </div>
            <FileUpload />
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-2xl font-semibold text-white">Uploaded Calls</h2>
            </div>
            {!loading && transcripts.length > 0 && (
              <div className="text-sm text-zinc-400 bg-zinc-800 px-3 py-1 rounded-full">
                {transcripts.length} {transcripts.length === 1 ? "call" : "calls"}
              </div>
            )}
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <h3 className="text-xl font-semibold text-zinc-300 mb-2">No transcripts yet</h3>
              <p className="text-zinc-500 max-w-md mx-auto">
                Upload your first audio file to get started with transcription and analysis.
              </p>
            </div>
          ) : (
            <div className="grid gap-6">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-4 text-sm text-zinc-400">
                          <div className="flex items-center space-x-1">
                            ...
                            <span>Uploaded: {new Date(item.uploadedAt).toLocaleString()}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="p-6 space-y-6">
                    ...
                    {item.scores ? (
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                          {[
                            { label: "Call Opening", value: item.scores.callOpening },
                            { label: "Issue Capture", value: item.scores.issueCapture },
                            { label: "Agent Sentiment", value: item.scores.agentSentiment, isText: true },
                            { label: "Customer Sentiment", value: item.scores.customerSentiment, isText: true },
                            { label: "CSAT/FCR", value: item.scores.csatFcr },
                            { label: "Resolution Quality", value: item.scores.resolutionQuality },
                          ].map((score, idx) => (
                            <div
                              key={idx}
                              className={`p-4 rounded-lg border ${score.isText ? "bg-zinc-800/30 border-zinc-700" : getScoreBgColor(score.value)}`}
                            >
                              <div className="flex items-center justify-between">
                                <span className="text-sm font-medium text-zinc-300">{score.label}</span>
                                {score.isText ? (
                                  <span className="text-sm font-medium text-zinc-300">{score.value}</span>
                                ) : (
                      ...
                      <div className="bg-zinc-800/30 rounded-lg p-6 border border-zinc-700 text-center">
                        ...
                        <p className="text-zinc-500 italic">Analysis data not available for this transcript.</p>
                      </div>
                    )}
                      ...
                      (item.coaching.summary ||
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-4">
                            ...
                            >
                              <span>{expandedCoaching[item._id] ? "Hide" : "Show"} Details</span>
                              <svg
                                className={`w-4 h-4 transition-transform ${expandedCoaching[item._id] ? "rotate-180" : ""}`}
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                              >
                          ...
                          {item.coaching.summary && (
                            <div className="bg-gradient-to-r from-orange-900/20 to-red-900/20 rounded-lg p-4 border border-orange-800/30 mb-4">
                              ...
                              <p className="text-zinc-300 text-sm leading-relaxed">{item.coaching.summary}</p>
                            </div>
                          )}
                          ...
                          {expandedCoaching[item._id] && (
                            <div className="space-y-6">
                              {/* Learning Resources */}
                              {item.coaching.resources && item.coaching.resources.length > 0 && (
                                <div>
                                  <h5 className="text-md font-semibold text-white mb-3 flex items-center">
                                    <svg
                                      className="w-4 h-4 mr-2 text-blue-400"
                                  ...
                                  <div className="grid gap-3">
                                    {item.coaching.resources.map((resource, idx) => (
                                      <div
                                        key={idx}
                                        className="bg-zinc-800/50 rounded-lg p-4 border border-zinc-700 hover:border-zinc-600 transition-colors"
                                      >
                                        <div className="flex items-center justify-between">
                                          <div className="flex-1">
                                            <h6 className="text-sm font-medium text-white mb-1">{resource.title}</h6>
                                            <p className="text-xs text-zinc-400">{resource.url}</p>
                                          </div>
                                          <a
                                            href={resource.url}
                                            target="_blank"
                              ...
                              {/* Quiz Section */}
                              {item.coaching.quiz && item.coaching.quiz.length > 0 && (
                                <div>
                                  <h5 className="text-md font-semibold text-white mb-3 flex items-center">
                                    <svg
                                      className="w-4 h-4 mr-2 text-purple-400"
                                  ...
                                  <div className="bg-zinc-800/30 rounded-lg p-4 border border-zinc-700 space-y-4">
                                    {item.coaching.quiz.map((question, qIdx) => (
                                          ...
                                          {question.options.map((option, oIdx) => (
                                            <label key={oIdx} className="flex items-center space-x-2 cursor-pointer">
                                              <input
                                                ...
                                                value={option}
                                                onChange={(e) =>
                                                  handleQuizAnswer(item._id, question._id, e.target.value)
                                                }
                                                className="text-purple-500 focus:ring-purple-500"
                                                disabled={quizResults[item._id]}
                                              />
                                    ...
                                    {!quizResults[item._id] ? (
                                      <button
                                        onClick={() => submitQuiz(item._id, item.coaching.quiz)}
                                        className="w-full mt-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors font-medium"
                                      >
                                        Submit Quiz
                                      </button>
                                      ...
                                      <div
                                        className={`mt-4 p-3 rounded-lg border ${quizResults[item._id].passed ? "bg-green-900/20 border-green-800" : "bg-red-900/20 border-red-800"}`}
                                      >
                                        <div className="flex items-center space-x-2">
                                          {quizResults[item._id].passed ? (
                                            <svg
                                          ...
                                          <span
                                            className={`font-medium ${quizResults[item._id].passed ? "text-green-300" : "text-red-300"}`}
                                          >
                                            Quiz {quizResults[item._id].passed ? "Passed" : "Failed"}:{" "}
                                            {quizResults[item._id].score}% ({quizResults[item._id].correct}/
                                            {quizResults[item._id].total})
                                          </span>
                                        </div>
                                        {item.coaching.completionCriteria && (
                                          <p className="text-xs text-zinc-400 mt-2">
                                            <strong>Completion Criteria:</strong> {item.coaching.completionCriteria}
                                          </p>
                                        )}
                                      </div>
                                    )}
